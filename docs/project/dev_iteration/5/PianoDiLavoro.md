...
| FASE 4 - Backend social login Google/Microsoft per utenti `User` | **Pianificata** | - | OIDC Authorization Code + PKCE; Google aperto a email verificate, Microsoft limitato a `issgreppi.it`; exchange code e blocco ruoli elevati |
...
| FASE 8 - Test automatici estesi auth/security | **Pianificata** | - | Integration test per password, social, admin utenti, RBAC, audit, token replay e dominio Microsoft |
...
| FASE 10 - Documentazione finale | **Pianificata** | - | Aggiornamento `status.md`, `changelog.md`, `.env.example` e checklist piano |
...
- accesso social per utenti normali `User` tramite Google e Microsoft, con Google aperto agli account Google con email verificata e Microsoft limitato al dominio/tenant scolastico `issgreppi.it`;
...
- Social login Microsoft Entra ID per utenti normali `User` del dominio/tenant scolastico `issgreppi.it`.
- Validazione backend dei token OIDC e, solo per Microsoft, del dominio/tenant.
- Collegamento sicuro di un provider esterno a un account `User` esistente con email verificata.
- Autocreazione account `User` da social login se l'email verificata non è ancora registrata e il provider soddisfa le proprie regole.
...
- Accesso Microsoft consumer/generico o tramite tenant Microsoft diversi da quello scolastico.
...
11. Per Microsoft il dominio/tenant `issgreppi.it` deve essere validato lato backend usando claim attendibili del provider, non input frontend; per Google si richiede email verificata senza vincolo di dominio.
...
- Per CineBase Google è un provider pubblico per utenti `User`: sono ammessi account Google con qualunque dominio email, purché Google certifichi quell'indirizzo con `email_verified = true`. Microsoft è invece un provider scolastico vincolato a `issgreppi.it`/tenant Entra ID.
...
- Microsoft OpenID Connect per tenant/dominio scolastico.
...
- Creazione automatica di un account `User` quando l'email Microsoft verificata `@issgreppi.it` non esiste.
- Collegamento al profilo esistente se l'email verificata appartiene a un account `User`.
- Rifiuto esplicito del login Microsoft se l'email/tenant non appartiene a `issgreppi.it`.
...
- Conservazione minima dei dati provider: provider, subject/oid, tenant id quando disponibile, email verificata, timestamp collegamento e ultimo accesso.
...
- Microsoft: validare `iss`, `aud`, firma, `exp`, `tid`, `oid`/`sub`, `preferred_username` o `email`; configurare e validare tenant/dominio scolastico `issgreppi.it`. Non accettare account consumer personali o tenant esterni.

Nota Microsoft:

- Il vincolo `issgreppi.it` è intenzionale solo per Microsoft/Entra ID: evita il caso multi-tenant/generico che richiederebbe configurazioni e trust publisher più complessi. Google resta invece disponibile per normali account Google con email verificata.
...
- `login.html`:
  ...
  - messaggi chiari per dominio Microsoft non autorizzato o account elevato che deve usare password locale.
- `registrazione.html`:
  - resta registrazione locale solo `User`;
  - può offrire gli stessi bottoni social, specificando che Google è aperto agli account Google verificati e Microsoft è riservato agli account scolastici.
...
- messaggi chiari per ultimo admin, account elevato, dominio Microsoft non scolastico e sessioni revocate.
...
- il backend deve essere l'unico punto che valida provider, email verificata, tenant Microsoft e linking account;
...
6. Backend valida state, scambia code e valida ID token secondo le regole del provider: Google richiede email verificata, Microsoft richiede dominio/tenant scolastico.
...
- le regole provider devono essere verificate lato backend: email verificata per Google, dominio/tenant scolastico per Microsoft;
...
- `User` può autenticarsi con password locale, Google o Microsoft; Microsoft è limitato agli account scolastici, mentre Google è consentito su qualunque dominio email se Google restituisce un'identità valida con `email_verified = true`.
...
MICROSOFT_TENANT_ID=<tenant_id_or_organizations>
...
**Obiettivo**: aggiungere accesso social sicuro per utenti normali, con Google aperto agli account Google verificati e Microsoft limitato al dominio/tenant scolastico.
...
6. Implementare callback provider:
   ...
   - per Microsoft, valida dominio `issgreppi.it` e tenant Microsoft se configurato;
...
- fake Microsoft valido `utente@issgreppi.it`: crea `User`;
- Google valido con dominio non scolastico: accettato se `email_verified == true`;
- Microsoft con email/tenant non `issgreppi.it`: rifiutato;
- email Google non verificata: rifiutata;
- tenant Microsoft non ammesso: rifiutato;
...
- [ ] Dominio/tenant Microsoft `issgreppi.it` validato backend
...
- Microsoft valido dominio `issgreppi.it`;
- Google con email non verificata rifiutato;
- Microsoft con dominio esterno rifiutato;
- tenant Microsoft errato rifiutato;
...
- social login Microsoft reale con account `@issgreppi.it`, se tenant/config disponibili;
- social login Microsoft con dominio/tenant non ammesso rifiutato, se testabile;
...
- regole provider applicate: Google email verificata, Microsoft dominio/tenant ammesso;
...
- `docs/project/status.md`
- `docs/project/changelog.md`
- eventuale tutorial futuro su setup Google/Microsoft OAuth, se richiesto.
...
| Email/domain spoofing provider | Media | Alto | Validare ID token provider; per Google richiedere `email_verified`, per Microsoft validare tenant/dominio esatto |
| Account takeover tramite linking email non verificata | Media | Alto | Collegare solo email verificata e provider attendibile; Microsoft vincolato a tenant scolastico |
...
| Microsoft claim email non uniforme | Media | Medio | Usare `preferred_username`, tenant ID e `oid`; documentare claim supportati; test fake |
| Google aperto a domini non scolastici crea utenti esterni | Media | Medio | Consentito dal requisito: assegnare sempre e solo ruolo `User`, nessuna elevazione senza password locale e audit su provider |
...
| Microsoft social | dominio `issgreppi.it`, tenant valido, tenant errato, account consumer rifiutato |
...
3. Social login Microsoft crea o collega solo utenti `User` con email `@issgreppi.it` e tenant/dominio validato.
4. Social login Microsoft con dominio/tenant non ammesso viene rifiutato.
...
Obiettivo: aggiungere gestione credenziali completa, recupero password via email, social login Google aperto agli account Google verificati, social login Microsoft limitato agli utenti normali del dominio/tenant `issgreppi.it`, e strumenti Admin per creare o promuovere PowerUser/Admin in modo sicuro.
...
5. social login backend Google/Microsoft con OIDC, Google senza vincolo dominio, Microsoft con dominio scolastico e blocco ruoli elevati;
