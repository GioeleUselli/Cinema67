...
- Area admin operativa: **completata**, `sale.html` con editor piantina, `proiezioni.html` workspace show multi-sala, `ricarica-credito.html` con ricerca e storico, `validazione-biglietti.html` con scanner QR/barcode e modalità Auto Click per validazione rapida, sidebar admin unificata con nuova sezione operativa
- **Iterazione 4 completata al 100%** — tutte le 17 fasi (1-13.2 incluse 9.1, 11.1, 12.1, 13.1, 13.2) completate e verificate
- **Debito tecnico residuo**: entità backend `Proiezione`/`Prenotazione` mantenute nello schema DB (modello, service, DI); endpoint `GET /proiezioni` ancora attivo come read-only compat; da rimuovere nella prossima iterazione con migration dedicata
